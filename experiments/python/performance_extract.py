...
def getMandatorSporadesPerformance(root, initClient, numClients):
    ...
    for cl in list(range(initClient, initClient + numClients, 1)):
        ...
        if content[0].strip().split(" ")[0] == "Warning:":
            content = content[1:]
        if not (content[3].strip().split(" ")[0] == "Throughput" and content[4].strip().split(" ")[0] == "Median"):
            sys.exit("Error in " + file_name + "\n")

        throughputs.append(float(content[3].strip().split(" ")[2]))
        medians.append(float(content[4].strip().split(" ")[3]))
        ninety9s.append(float(content[5].strip().split(" ")[4]))
        errors.append(float(content[6].strip().split(" ")[3]))

...
func (b *Benchmark) Execute(memBlock *proto.MemPool) *proto.MemPool {
	var commands []*proto.ClientBatch
	if b.mode == 0 {
		commands = b.residentExecute(memBlock.ClientBatches)
	} else {
		commands = b.redisExecute(memBlock.ClientBatches)
	}
	return &proto.MemPool{
		Sender:        memBlock.Sender,
		UniqueId:      memBlock.UniqueId,
		Type:          memBlock.Type,
		Note:          memBlock.Note,
		ClientBatches: commands,
		RoundNumber:   memBlock.RoundNumber,
		ParentBlockId: memBlock.ParentBlockId,
		Creator:       memBlock.Creator,
	}
}

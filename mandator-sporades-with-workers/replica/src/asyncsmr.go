...
func (rp *Replica) updateSMR() {
	...
	// check if all the required mem blocks and mini mem blocks exist.
	...
	for i := 0; i < len(toCommit); i++ {
		...
		for j := 0; j < rp.numReplicas; j++ {
			if int(nextMemBlockLogPositionsToCommit[j]) > rp.asyncConsensus.lastCommittedRounds[j] {
				...
				// if the block exists, check all the mini blocks, and if anything is missing send an external mini block request
				...
				for k := startMemPoolCounter; k <= lastMemPoolCounter; k++ {
					...
					if ok {
						...
						for l := 0; l < len(miniBlockIDs); l++ {
							...
							_, ok := rp.memPool.miniMap.Get(miniBlockID)
							if !ok {
								rp.sendExternalMiniBlockRequest(miniBlockID)
								common.Debug("Mini block with id "+miniBlockID+" does not exist, sending an external request", 0, rp.debugLevel, rp.debugOn)
								readyToCommit = false
							}
						}
					} else {
	...
	if readyToCommit {
		...
		for i := 0; i < len(toCommit); i++ {
				...
				if int(nextMemBlockLogPositionsToCommit[j]) > rp.asyncConsensus.lastCommittedRounds[j] {
					...
					for k := startMemPoolCounter; k <= lastMemPoolCounter; k++ {
						...
						for l := 0; l < len(miniBlockIDs); l++ {
							...
							minimemPoolClientResponse := rp.updateApplicationLogic(miniBlock)
							common.Debug("Committed mini block "+miniBlockID, 1, rp.debugLevel, rp.debugOn)
			...
			common.Debug("Committed async consensus block "+nextBlockToCommit.Id+" with mem pool indexes "+fmt.Sprintf("%v", nextBlockToCommit.Commands)+" at time "+fmt.Sprintf("%v", time.Now().Sub(rp.asyncConsensus.startTime)), 4, rp.debugLevel, rp.debugOn)

...
type AsyncConsensus struct {
	...
	consensusPool   *common.AsyncConsensusStore        // an id of a consensus block is creator_name.v.r.type.level. type can be r (regular) or f (fallback), level can be 1,2 or -1 (for regular blocks)
...
func InitAsyncConsensus(debugLevel int, debugOn bool, numReplicas int) *AsyncConsensus {
	...
	asyncConsensus := AsyncConsensus{
		...
		consensusPool:       &common.AsyncConsensusStore{},
		isAsync:             false,
...
func (rp *Replica) sendGenesisConsensusVote() {
	...
	common.Debug("Sent boot strap consensus vote to "+strconv.Itoa(int(nextLeader)), 1, rp.debugLevel, rp.debugOn)

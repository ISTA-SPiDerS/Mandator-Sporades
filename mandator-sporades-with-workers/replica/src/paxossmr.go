...
func (rp *Replica) updatePaxosSMR() {
	...
	if readyToCommit {
		...
		for i := rp.paxosConsensus.lastCommittedLogIndex + 1; i < rp.paxosConsensus.lastDecidedLogIndex+1; i++ {
			...
			common.Debug("Committed paxos consensus instance "+strconv.Itoa(int(rp.paxosConsensus.view))+"."+strconv.Itoa(int(i))+" with mem pool indexes "+fmt.Sprintf("%v", nextInstanceToCommit.decisions)+" at time "+fmt.Sprintf("%v", time.Now().Sub(rp.paxosConsensus.startTime)), 4, rp.debugLevel, rp.debugOn)

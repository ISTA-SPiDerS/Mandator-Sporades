...
/*
	Implements a block store for the mem pool
*/
...
func (ms *MessageStore) Init(debugLevel int, debugOn bool) {
	...
	ms.debugOn = debugOn
	Debug("Initialized a new mem block message store", 0, ms.debugLevel, ms.debugOn)
}
...
func (ms *MessageStore) Add(block *proto.MemPool) {
	...
	if !ok {
		...
		Debug("Added a new block to store with id "+block.UniqueId, 0, ms.debugLevel, ms.debugOn)
	}
...
func (ms *MessageStore) Get(id string) (*proto.MemPool, bool) {
	...
	if !ok {
		Debug("Requested mem block does not exist, hence returning nil for id "+id, 0, ms.debugLevel, ms.debugOn)
		return nil, ok
	} else {
		Debug("Requested mem block exists, hence returning the block for id "+id, 0, ms.debugLevel, ms.debugOn)
...
func (ms *MessageStore) Remove(id string) {
	...
	Debug("Removed a mem block with id "+id, 0, ms.debugLevel, ms.debugOn)
}
...
func (ms *MessageStore) AddAck(id string, node int32) {
	...
	if ok {
		...
		Debug("Added an ack for mem block with id "+id, 0, ms.debugLevel, ms.debugOn)
	}
	Debug("Adding an ack failed for mem block with id "+id+" because the mem block does not exist", 0, ms.debugLevel, ms.debugOn)
}

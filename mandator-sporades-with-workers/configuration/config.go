...
// InstanceConfig describes the set of peers, workers and clients in the system
type InstanceConfig struct {
	Peers   []Instance `yaml:"peers"`
	Clients []Instance `yaml:"clients"`
	Workers []Instance `yaml:"workers"`
}
...
func NewInstanceConfig(fname string, name int64) (*InstanceConfig, error) {
	...
	// sanity checks
	allInstances := append(cfg.Peers, cfg.Workers...)
	allInstances = append(allInstances, cfg.Clients...)
...
func configureSelfIP(cfg InstanceConfig, name int64) InstanceConfig {
	...
	for i := 0; i < len(cfg.Workers); i++ {
		if cfg.Workers[i].Name == strconv.Itoa(int(name)) {
			cfg.Workers[i].Address = "0.0.0.0:" + getPort(cfg.Workers[i].Address)
			return cfg
		}
	}
	return cfg
